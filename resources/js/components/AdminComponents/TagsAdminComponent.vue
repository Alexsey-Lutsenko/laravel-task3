<template>
    <div class="d-flex flex-column align-items-center">
        ...
        <div class="my-5 w-50">
            ...
            <DataTable :value="tags" dataKey="id" :loading="loading" :resizableColumns="true" columnResizeMode="fit"
        ...
        <Dialog v-model:visible="tagDialog" :style="{width: '450px'}" header="Управление Тегами" :modal="true" class="p-fluid">
            <div class="field">
                ...
                <InputText id="tag" v-model.trim="tag.tag" required="true" autofocus :class="{'p-invalid': submitted && (!tag.tag || errors.tag)}" />
        ...
        <Dialog v-model:visible="deleteTagDialog" :style="{width: '450px'}" header="Внимание" :modal="true">
            <div class="confirmation-content d-flex align-items-center">
                ...
                <span v-if="tag">Вы точно хотите удалить <b>{{ tag.tag }}</b>?</span>
            </div>
            <template #footer>
...
export default {
    ...
    setup() {
        ...
        const openNew = () => {
            ...
            const resizer = document.getElementsByClassName('p-column-resizer-helper')[0]

            console.log(resizer.style.display)
        };
